...
public class DemoteUserStrategy implements UserCommandStrategy {
    ...
    public boolean supports(String command) {
        return "demote".equalsIgnoreCase(command);
    }
    ...
    public UserResponseDTO execute(UserCommandDTO dto) {
        ...
        if (!Role.ADMIN.equals(actingUser.getUserRole())) {
            throw new SecurityException("Access denied: only admins can demote users.");
        }
        ...
        if (Role.USER.equals(target.getUserRole())) {
            throw new IllegalStateException("User " + dto.getEmail() + " is not a moderator.");
        }

        Role previousRole = target.getUserRole();
        target.setUserRole(Role.USER);
